...
/**
 * This class registers tracing components into {@link ClientBuilder}. It is required to call
 * {@link #configure(ClientBuilder)} or its variants to enable tracing in {@link jakarta.ws.rs.client.Client}, however
 * implementation might enable tracing globally.
 *
 * Invoking {@link ClientTracingRegistrar#configure(ClientBuilder)} returns a {@link ClientBuilder} with enabled tracing
 * integration. Note that following calls to {@link ClientBuilder} which change {@link ExecutorService} might break
 * tracing integration. If a custom {@link ExecutorService} has to be used use
 * {@link ClientTracingRegistrar#configure(ClientBuilder, ExecutorService)}.
 *
 * @author Pavol Loffay
 */
