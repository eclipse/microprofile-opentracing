...
3. The MicroProfile implementation must provide a mechanism to automatically start a Span for any incoming JAX-RS request.
...
5. The MicroProfile implementation must provide a mechanism to automatically start a Span for any outgoing JAX-RS request.
...
An [Tags.ERROR tag](https://github.com/opentracing/specification/blob/master/semantic_conventions.md#span-tags-table) SHOULD be added to a Span on failed operations for any server error (5xx) codes.
...
An Tags.ERROR tag SHOULD be added to a Span on failed operations. It means for any client error (4xx) codes.
If there is an exception object available the implementation SHOULD also add logs `event=error` and `error.object=<error object instance>` to the active span.
...
+
Even when the `@Traced(false)` annotation is used for a JAX-RS endpoint method, the upstream SpanContext will still be extracted if it exists.
The extracted SpanContext will be used to define a current Span.

* operationName=<Name for the Span>.
Default is "".
If the `@Traced` annotation finds the operationName as "", the default operationName is used (see <<server-span-name>>).
