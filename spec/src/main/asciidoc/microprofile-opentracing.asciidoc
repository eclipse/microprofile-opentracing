...
The operation name can be configured via MicroProfile Config configuration property `mp.opentracing.server.operation-name-provider`.
...
if the class is annotated with `@Path(service)` and method `@(endpoint)` then the operation name is `/service/endpoint`.
...
Any exceptions thrown by non JAX-RS components have to be logged to the span corresponding to the ongoing invocation.
The span should be annotated with the following data:
