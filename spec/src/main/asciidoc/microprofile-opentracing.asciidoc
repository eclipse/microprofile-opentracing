...
If `operationName` is specified on a class, then the operation name of each traced method in that class is prefixed with the class `operationName` followed by a forward slash (`/`).
If the class encloses a JAX-RS endpoint, the prefix is added after `<HTTP method>:` and before `<package name>`.
If the JAX-RS endpoint has an explicit `operationName`, then the class `operationName` along with `/` is prefixed before the method's `operationName`.
